        ...
        <div className="col-sm-12 col-md-6 col-lg-3 my-3">
            <div className="card p-3 rounded">
                <img src={fooditem.images[0].url}
                    alt={fooditem.name} className="card-img-top mx-auto" />

                <div className="card-body d-flex flex-column">
                    <h5 className="card-title">{fooditem.name}</h5>
                    <p className="food_item_des">{fooditem.description}</p>
                    ...
                    {
                        !showButtons ? (
                            <button type="button" id="cart_btn" className="btn btn-primary d-inline ml-4" onClick={addToCartHandler}>
                                Add to Cart
                            </button>
                        ) : (

                            <div className="stockCounter d-inline">
                                <span className="btn btn-daner minus" onClick={decreaseQty}>
                                    -
                                </span>
                    ...
                    <br />
                    <p>Status:{" "}<span id="stcok_status" className={fooditem.stock ? "greenColor" : "redColor"}>{fooditem.stock ? "In Stock" : "out of stock"}</span></p>
                </div>
            </div>
        </div>
    )
